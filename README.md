...
Agent 任务执行与回放控制台。输入一个目标，Agent 生成执行计划、调用工具、流式输出结果；任务详情页按对话维度展示助手消息、工具调用和工具结果，回放页也可以随进度逐步查看同一份对话。你可以按工作区组织任务，对需要人工确认的工具调用进行审批，也可以暂停、继续、取消、重跑，并查看完整事件回放和统计。
...
- 协作：工作区分组、人工审批（`APPROVAL_ENABLED=true` 开启）
...
开启人工审批后，需要审批的工具会暂停执行并向前端推送 `approval.requested` 事件：
...
- 增加更多本地工具：`read_file`、`write_report`、`export_markdown`
